...
export function ToDoList() {
  ...
  function handleCheckboxClick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const itemIndex = itemCopy.findIndex((item) => item.name === itemName);
    itemCopy[itemIndex] = { name: itemName, isPurchased: checkbox.checked };

    const uncheckedItems = itemCopy.filter((item) => !item.isPurchased);
    const checkedItems = itemCopy.filter((item) => item.isPurchased);
  ...
  return (
    <div>
      <input
        ...
        name={item.name}
        data-testid={item.name} //FOR TESTING
      />
      {item.name}
    </div>
  );
